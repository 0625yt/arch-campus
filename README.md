...
강의자료 업로드 → 요약·자료 질문 → 연습 문제 → 채점·오답 복습을 연결한다. 학기별 시간표와 수강 과목의 학점·등급·누적 평점을 한곳에서 관리한다.
...
| 영역 | 구현 내용 |
|---|---|
| 내 캠퍼스 | 학기 선택, 주간/오늘/목록 시간표, 강의실·교수·수업 시간, 과목 수정, 우선순위·예정 일정 |
| 성적 | 학기별 과목·0학점 포함 이수학점·등급, 4.5 만점 학기/누적 평점, P/NP 처리 |
| 공부 공간 | 학기 선택, 과목·교수·강의실 검색, 정규/개인 공부 분류, 최근 활동 이어가기 |
...
**아직 제공하지 않음:** FSRS 간격 반복, 푸시 알림, 외부 캘린더 양방향 동기화, 결제·사용량 과금, 친구 초대 보상, 합격·코호트 성과 통계, 팀플·진로 위저드. MFA 로그인 화면·서버 AAL2 검증과 DB 직접 접근 제한 정책은 운영 DB에 적용했다. 복구 코드는 아직 제공하지 않는다.
...
- 저장소에는 0001~0028 SQL 마이그레이션이 있다. 0028은 학기 식별·이수학점·등급을 추가하며 2026-09-25 운영 DB에 적용했다.
