...
**아직 제공하지 않음:** FSRS 간격 반복, 푸시 알림, 외부 캘린더 양방향 동기화, 결제·사용량 과금, 친구 초대 보상, 성적·합격 추적, 팀플·진로 위저드. MFA 로그인 화면·서버 AAL2 검증은 구현했다. DB 직접 접근까지 강제하는 0027 정책은 연결 정보 문제로 적용·검증 대기 중이다.
...
- 저장소에는 0001~0027 SQL 마이그레이션이 있다. 파일 존재는 운영 DB 적용 증거가 아니다.
