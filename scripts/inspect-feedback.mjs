...
const { data, error } = await supabase
  .from("feedback")
  .select(
    "id, owner_id, target_type, target_id, generation_id, rating, category, body, created_at",
  )
...
for (const f of data) {
  ...
  console.log(`   target=${f.target_id} owner=${f.owner_id?.slice(0, 8)} gen=${f.generation_id?.slice(0, 8) ?? "-"}`);
  console.log("");
}
