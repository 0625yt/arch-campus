 ...
 *
 * 정책:
 *  - 아직 다시 맞히지 않은 오답은 기간과 관계없이 모두 노출.
...
export default async function QuizWrongOnlyPage({
  ...
  const wrongItems = await listWrongItems({ ownerId, quizId, sinceDays: null, limit: 200 });
  const wrongIds = Array.from(new Set(wrongItems.map((w) => w.questionId)));
