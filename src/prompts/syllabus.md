...
> **모델**: Sonnet 4.6 (정확도 우선, 룰 + 서버 보정 + 사용자 검토로 최종 확인)
...
6. **주차표를 학기 시작일로만 계산하지 않는다.** course.schedule에 "월 09:00-11:50"처럼 강의 요일이 있으면, "3주차 시험", "Unit 7-9 확인" 같은 수업 중 활동은 그 주의 실제 강의 요일에 맞춘다. 학기 시작일이 화요일이라도 수업이 월요일이면 화요일로 밀지 않는다.
7. **제출·LMS·온라인 마감은 강의 요일로 억지 보정하지 않는다.** "LMS 입력", "제출", "업로드", "마감", "까지"처럼 별도 마감 표현이 있으면 본문 날짜를 우선한다.
8. **course.schedule 추출을 대충 넘기지 않는다.** 본문·표·헤더에 요일/시간이 보이면 가능한 한 모두 뽑는다. 이 값이 주차표 날짜 계산의 기준이다.
...
- 강의계획서에 주차표만 있고 절대 날짜가 없으면, course.schedule의 요일과 termStart를 함께 써서 해당 주의 강의일을 계산한다. 그래도 모호하면 notes에 "강의 요일 확인 필요"를 남기고 confidence를 낮춘다.
