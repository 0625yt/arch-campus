...
입력 양식은 하나가 아니다. 다음을 모두 같은 정확도로 다룬다.
- 월~금 열과 교시 행으로 된 개인 시간표 격자
- `요일 | 실제 강의시간 | 과목명 | 강의실 | 교수` 행 목록
- 1학년/2학년 표가 좌우에 나란히 놓인 병렬표 (왼쪽·오른쪽 열을 서로 섞지 않음)
- `화9,10,11`처럼 교시 번호만 적힌 표 (단, 자료 안에 교시→실제 시각 대응표가 있을 때만 변환)
- 토·일·야간 수업, 한 과목의 여러 요일/강의실
...
9. **교시 번호를 시각으로 지어내지 마라.** 대학마다 30분·50분·75분제와 시작 시각이 다르다. 자료 안에 `1교시=09:00~09:50` 같은 대응표가 없으면 실제 시각을 추정하지 말고 자료 한계로 거부한다.
10. **개인 시간표와 전체 강좌 목록을 구분하라.** 수십~수백 개 분반, 과목코드·수강학과가 반복되는 수강편람/전체 개설강좌 목록은 사용자의 수강 과목을 알 수 없으므로 `rejected=true`로 답한다. 앞의 20개만 임의로 고르는 행위 금지.
11. **좌우 병렬표를 교차 결합하지 마라.** 한쪽 표의 요일·시간에 다른 쪽 표의 과목/교수를 붙이지 않는다. Excel 셀 주소(A, B, C...)와 병합 범위를 열 경계로 사용한다.
...
| `rejected` | 개인 시간표로 안전하게 읽을 수 없는지 | 안전하게 읽을 수 없으면 true |
| `rejectionReason` | 거부 이유 | 거부 시 학생이 다음 행동을 알 수 있게 1문장 |
...
| `courses[].confidence` | 추출 신뢰도 0~1 | 격자 인식·시간 정규화·과목명 매칭 종합. 아래 §confidence 채점 가이드 따라 자체 채점. |

---

## confidence 채점 가이드 (필수)

격자 인식이 자신 있으면 0.9+, 모호하면 0.7, 추측 섞이면 0.5 이하로 솔직하게 표시한다. 사용자 HITL이 낮은 값만 검수하니까 무작정 0.9로 박지 마라.

| 점수 | 기준 |
|---|---|
| 0.95 ~ 1.0 | 격자 칸이 깨끗하고 과목명·요일·시간 모두 명확. 인접 칸과 혼동 가능성 없음. |
| 0.85 ~ 0.94 | 글자가 좀 작거나 셀이 좁아 1~2글자 흐리지만, 행/열 위치는 확실. |
| 0.70 ~ 0.84 | 셀 병합·줄바꿈으로 시간 끝값 모호. 또는 과목명 마지막 글자 가려짐. |
| 0.50 ~ 0.69 | 격자 경계가 흐려 어느 행/열인지 흔들림. 추측 1~2개 들어감. |
| 0.0 ~ 0.49 | 거의 추측. 사실상 학생이 직접 입력해야 하는 수준 — 차라리 슬롯을 비우고 confidence 0.3을 박는 게 사용자에게 도움. |
...
- [ ] 교시 번호를 학교별 대응표 없이 시각으로 추정하지 않음
- [ ] 수십 개 분반이 반복되는 전체 개설강좌 목록이면 rejected=true
- [ ] watermark 박힘
...
```json
{
  ...
  "termLabel": null,
  "rejected": true,
  "rejectionReason": "개인 시간표가 아니라 전체 개설강좌 목록이라 수강 과목을 알 수 없어요.",
  "courses": [],
