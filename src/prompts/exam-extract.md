...
위 7개 중 하나라도 어기면 학생이 "이 자료에 없는 답을 AI가 만들어 풀어주는 앱"으로 인식 → 학교 차단 + 서비스 종료.
...
- 한 자료에서 최대 50개까지 추출. 그 이상이면 처음 50개만 박고 `truncated: true`.
- 50개 넘는 자료는 학생이 페이지 범위 지정해 다시 추출하는 흐름(향후 PR).
