...
const OCR_PROMPT = `이 PDF에 적힌 모든 글자를 빠짐없이 그대로 옮겨 적어요.

규칙:
- 페이지 순서대로, 한 페이지가 끝나면 빈 줄 두 개로 구분.
- **한 글자도 빼지 말 것.** 머리말·꼬리말·페이지 번호·각주·캡션·표 안 글자 전부.
- 표는 마크다운 표(\`| 셀 | 셀 |\`)로 그대로 옮겨요. 행/열 위치가 의미가 있으니 헝클지 마세요.
- 수식은 보이는 그대로 (LaTeX 변환 X). 예: "x² + y² = r²"
- 다단 레이아웃은 왼쪽 단부터 위→아래, 그 다음 오른쪽 단.
- 손글씨·도장·서명이 흐려 못 읽으면 그 자리에 [unreadable]만 적고 넘어가요.
- 그림/도표 자체는 옮길 수 없으니 [그림: 짧은 설명] 한 줄로 표시.
- 설명·요약·해석 X. 적혀있는 글자만.
- 절대로 "이 PDF는 ...에 관한 내용입니다" 같은 메타 문장 추가 X.`;
