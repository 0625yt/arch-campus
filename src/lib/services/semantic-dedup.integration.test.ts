  ...
  it(
    ...
    async () => {
      ...
      for (const [a, b, label] of pairs) {
        console.log(`  ${label}: ${(cosineSimilarity(emb.embeddings[a], emb.embeddings[b]) * 100).toFixed(1)}%`);
      }

      const result = await dedupeBySemantics(items);
      console.log("\n=== 의미 dedup 결과 ===");
      console.log(`입력 ${items.length} → 유지 ${result.kept.length} / 제거 ${result.dropped.length}`);
