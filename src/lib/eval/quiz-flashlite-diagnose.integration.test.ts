...
describe.runIf(shouldRun)("Flash-Lite evidence 실패 원인 진단", () => {
  it(
    ...
    async () => {
      ...
      console.log(`총 생성 ${total}개 · evidence 통과 ${kept}개 (${Math.round((kept / total) * 100)}%)`);
      console.log(`\ndrop 사유별:`);
      console.log(`  A. evidence 비어있음     : ${sujiBuckets.비어있음}  ← 프롬프트로 강제 가능`);
      console.log(`  B. 너무 짧음(<10자)      : ${sujiBuckets.너무짧음}  ← 프롬프트로 최소길이 요구 가능`);
      console.log(`  C. 본문 인용 아님(환각)  : ${sujiBuckets.인용아님_환각}  ← 근본적, 살리기 어려움`);
