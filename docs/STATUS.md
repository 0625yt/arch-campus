...
### 캘린더 듀얼 뷰 (2026-05-12 요청)
캘린더 페이지에 토글/탭 추가:
- **시간표 뷰** — 주간 반복 수업만 (kind=class, courses 기반)
- **내 일정 뷰** — 시험·과제·발표·기타 (현재 보이는 그 뷰)
현재 캘린더 UI/UX는 유지하면서 데이터 필터링만 두 모드로 분기. 모바일 calendar-board의 색점 표시도 모드별로.
