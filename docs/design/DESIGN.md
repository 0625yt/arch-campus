# DESIGN.md — arch-campus 디자인 가이드

> Current product-specific style command: [ARCH-CAMPUS-STYLE.md](ARCH-CAMPUS-STYLE.md)

> **사람이 만든 것처럼 보이는** 깔끔한 UI/UX. ChatGPT·Claude·Linear·Granola 톤.
> **반응형 1급 (Mobile · iPad · Desktop 동등)**. 대학생은 폰(지하철)·태블릿(카페·도서관)·노트북(스터디룸·집) 다 씀.
> Bitjaru StyleSeed의 69개 룰 중 arch-campus(학습 SaaS, 한국어, 라이트모드)에 맞는 것만 압축. 원본: [STYLESEED-FULL.md](STYLESEED-FULL.md). Linear 토큰: [styleseed-linear-tokens.css](styleseed-linear-tokens.css).

---

## §A. Apple HIG 적용 (2026-05-13 추가) ★★★

> developer.apple.com/kr/design + developer.apple.com/design/human-interface-guidelines 정독 후 우리 사이트에 적용한 원칙.
>
> **트리거 한 줄**: "로그인은 좋은데 안에 들어가면 칙칙하다" — Apple은 종이 위에 **빛**을 깐다. 우리는 그동안 종이 위에 **회색 카드**만 깔고 있었다.

### A-1. Apple Foundations 핵심 5개

| 원칙 (HIG) | 우리에게 적용 |
|---|---|
| **Materials** — 표면은 단일 솔리드 X. 빛·반투명·미세 그라데이션으로 깊이. | 페이지 배경에 4-blob radial gradient + 카드는 흰색 그대로. |
| **Color** — 시맨틱·미니멈. 의미 있는 곳에만, 면적은 최소. | kind tint(파스텔)만 액센트. 큰 면을 색으로 채우는 것 금지. |
| **Typography** — SF Pro / 강한 점프 위계. | hero 48~56px → section 22~28px → body 14~15px → meta 11~12px. 그 사이 잘게 X. |
| **Layout** — 호흡. 한 화면에 정보 욱여넣지 않음. | 페이지 상단 padding `pt-12`로 통일, hero 아래 큰 여백, 카드 grid 12~16px gap. |
| **Motion** — 작고 빠른 ease-out, 200~300ms. | hover translate -1px + opacity, 200ms cubic-bezier(0.22, 1, 0.36, 1). |

### A-2. 로그인 화면이 잘 되는 이유 (우리가 살린 패턴)

분석: [src/app/login/page.tsx](../../src/app/login/page.tsx)

1. **배경 = 다층 radial blob**. 흰 종이 위에 cobalt 45%, 머스타드 40%, mauve 42%, cobalt-tail 20% — 사용자가 "컬러"라고 의식하기 전 단계의 미세 그라데이션.
2. **카드 X. 종이 위에 떠 있는 버튼 한 개**. 다층 그림자(`1px shadow + 12px soft glow + 1px ring`)로 표면감.
3. **타이포 점프 강함**. 38px hero → 14px sub → 13px meta. 14·15·16px 잡다 X.
4. **여백이 광활**. 좁은 max-w(420px) + 큰 vertical padding(py-16).
5. **인터랙션 마이크로**. hover에 1px 떠오름 + glow 강화, active에서 1px 누름 + 0.98 scale.

### A-3. 대시보드가 칙칙한 이유 (이번에 고치는 것)

1. ❌ 모든 페이지 배경이 단색 `#f5f5f7` 한 색.
2. ❌ 카드가 흰색 + 그림자 거의 X → 평평.
3. ❌ hero가 본문과 같은 면에 있음 → "올라온" 느낌 없음.
4. ❌ 카드 hover에 변화 없음 → 죽어있음.
5. ❌ 타이포 사이즈가 점프하지 않고 12·13·14·15·16·17 다 등장 → 위계 무너짐.

### A-4. 적용 명세 (이번 PR이 박는 것)

**(1) 페이지 표면 (Surface)**

```css
/* 토큰 — surface-canvas는 로그인 그라데이션의 "안쪽 화면" 버전 */
--surface-canvas: radial-gradient(
    ellipse 80% 50% at 0% 0%,
    rgba(122, 166, 214, 0.06),
    transparent 60%
  ),
  radial-gradient(
    ellipse 60% 40% at 100% 100%,
    rgba(176, 156, 200, 0.05),
    transparent 65%
  ),
  #fafafa;
```

`/dashboard/*` 모든 페이지 배경을 이걸로. **사용자가 보면 그냥 회색 같지만, 카메라로 찍어 비교하면 살아 있는 종이**.

**(2) 카드 elevation 3단계**

| Level | 용도 | 그림자 |
|---|---|---|
| `elev-1` | 기본 카드 (이번 주, 자료 카드) | `0 1px 2px rgba(0,0,0,0.04)` |
| `elev-2` | hero·focus 카드 (Today 큰 카드, 강의 카드) | `0 1px 2px rgba(0,0,0,0.04), 0 8px 24px -12px rgba(0,0,0,0.08)` |
| `elev-3` | floating·modal | `0 4px 12px rgba(0,0,0,0.06), 0 24px 56px -12px rgba(0,0,0,0.16)` |

**평소엔 elev-1. hover에 elev-2.** 그래야 "올라온다"가 느껴짐.

**(3) 타이포 점프 고정**

```
페이지 타이틀  56 / 48 / 34px  wght-620  letter-spacing -0.024em
섹션 타이틀   28 / 22px        wght-620  -0.012em
카드 타이틀   17~20px          wght-620  -0.012em
본문        14~15px           wght-450  -0.012em
메타        11~12px           wght-560  uppercase 0.06em
```

**중간 사이즈(16, 18) 쓸 거면 카드 타이틀이나 본문 어디에 속하는지 다시 판단**. 16px hero는 X.

**(4) 인터랙션**

- hover: `translate-y: -1px` + elev-1→elev-2 + 색 변화 1단계
- active: `scale: 0.98` 또는 `translate-y: 0`
- 모든 트랜지션 `200ms cubic-bezier(0.22, 1, 0.36, 1)` (`--ease-out`)
- 페이지 진입 fade-up은 이미 잘 됨 (0.3~0.4s stagger 100ms)

**(5) 헤더 영역의 "spacious" 룰**

페이지 진입 후 hero까지 `pt-12 sm:pt-16 md:pt-20`. 그 위는 topbar(breadcrumb·date) `12px` 라인 하나만. 이게 Apple 사이트 모든 페이지의 공통 호흡.

### A-5. 금지 — 이 PR 이후 다시 등장하면 안 되는 패턴

- 페이지 배경 단색 (`bg-[#f5f5f7]` 단독)
- 카드 hover 변화 없음
- 같은 페이지 안에 14·15·16·17 모두 등장
- hero와 카드가 동일한 표면 위 (hero가 안 떠 있음)
- 큰 면을 컬러로 채움 (예: 카드 전체 배경 코랄)

### A-6. 참고 페이지

- [https://developer.apple.com/kr/design/](https://developer.apple.com/kr/design/) — 메인 디자인 허브 (Liquid Glass, 몰입형 경험)
- [https://developer.apple.com/design/human-interface-guidelines/](https://developer.apple.com/design/human-interface-guidelines/) — Foundations 전체
- 우리 로그인 화면 [src/app/login/page.tsx](../../src/app/login/page.tsx) — Apple 톤 우리 사이트 내 유일 reference

---

## 0. 톤 한 줄

> **"AI가 만들었구나"가 아니라 "사람이 신경 써서 만들었네"로 읽혀야 한다.**

판단 기준:
- 시니어 디자이너가 봤을 때 "장식이 너무 많네" 안 나오나?
- 색이 한 개 이상 강조되고 있지 않나?
- 그림자·라운드·여백이 과하지 않나?

---

## 1. 색의 절제 (★ 가장 중요)

### 1-1. 액센트는 단 하나
- **블루 단일 (`#2563eb`, `--color-accent`)**. 활성 탭, 진행률 바, 1순위 CTA, 강조 링크에만.
- 본문 텍스트·배경·일반 보더에 액센트 색 X.
- 다른 색이 필요해 보이면 그건 **회색 단계로 위계를 못 만든 것**.

### 1-2. 임팩트 색은 작게, 강하게
| 역할 | 색 | 최대 크기 | 용도 |
|---|---|---|---|
| 마감 임박·경고 | `--color-warn` `#d97706` | 도트 6px + 텍스트 11px | "D-2" 배지 |
| 완료·상승 | `--color-success` `#059669` | 도트 6px + 텍스트 13px | 성적 향상 |
| 알림 | `#ef4444` | 도트 6px (텍스트 X) | 단일 도트만 |

큰 면적에 절대 X (배지·도트만).

### 1-3. 회색 5단계 — 검정(#000)은 절대 X
| 단계 | hex | 역할 |
|---|---|---|
| Strong | `#2A2A2A` (현재 `--color-fg` `#1a1a1a`) | 가장 강한 강조 (대시보드 큰 숫자, 헤딩) |
| Primary | `#3C3C3C` | 본문 메인 텍스트 |
| Secondary | `#6A6A6A` (현재 `--color-fg-muted` `#6b7280`) | 캡션·라벨 |
| Tertiary | `#7A7A7A` (현재 `--color-fg-subtle` `#9ca3af`) | 부제, 날짜, 설명 |
| Disabled | `#9B9B9B` | 비활성, placeholder |

**우리 globals.css 토큰은 이 단계와 거의 일치**. 새 색 추가 X. 위계가 부족하면 단계를 더 쓰지 새 색을 만들지 X.

### 1-4. 배경 위계
| 역할 | 색 |
|---|---|
| 페이지 | `#FAFAFA` 또는 `#FFFFFF` (현재: 흰색 메인 + 상단바 위 캔버스 blob) |
| 카드 | `#FFFFFF` |
| 비활성 trail | `--color-border` |
| 액센트 tint (선택 행) | `--color-accent-soft` `#eff6ff` |

---

## 2. 텍스트 위계 5단계

| 레벨 | 크기 | weight | 용도 |
|---|---|---|---|
| Display | 28px | 600 | 페이지 제목 ("좋은 오후예요…") |
| H1 | 20px | 600 | 카드 메인 텍스트 (5분 카드 본문) |
| H2 | 14~15px | 500 | 리스트 아이템 제목 |
| Body | 13~13.5px | 400 | 본문 |
| Caption | 12~12.5px | 400 | 부제, 날짜, 메타 |
| Micro | 11px | 500 | 배지, 섹션 라벨 (대문자) |

**규칙**:
- 한 화면에 6개 이상 크기 X
- 차이는 **2~4px**씩만. 12 → 14 → 16 → 20 → 28
- 한국어 + Pretendard라면 letter-spacing은 -0.01em ~ 0 (영문보다 살짝 좁게)

---

## 3. 숫자 표기 (학점·점수·진행률)

**큰 숫자 + 작은 단위, 2:1 비율**, 단위는 `ml-0.5` 정도 띄우기.
- 히어로 메트릭: 48px / 24px (예: `3.8M`)
- KPI 카드: 36px / 18px (예: `4.2/4.5`)
- 진행률 텍스트: 13px / 13px 같은 크기 (`5/12`)

폰트: `tabular-nums` 클래스 필수 (숫자 자리 정렬).

---

## 4. 그림자 — 거의 안 보이게

- 카드: `box-shadow: 0 1px 2px rgba(0,0,0,0.04);` 정도. 호버 시에만 살짝 더.
- **opacity 4~8% 이상이면 과함**.
- "그림자 보인다" 싶으면 이미 너무 많이 줬다.
- 보더(`1px var(--color-border)`)가 그림자보다 우선. **분리는 그림자가 아니라 배경 차이로**.

---

## 5. 카드 구조

```
┌──────────────────────────────┐
│ Header (메타·라벨)              │  - 12px secondary
│                                │
│ Content (메인 정보)            │  - 14~20px primary
│                                │
│ Footer (액션·진행률)           │  - 12~13px tertiary
└──────────────────────────────┘
```

**규칙**:
- 한 카드에 정보 4개 이상 X. 넘치면 두 카드로 분할.
- 제목·메타 사이 여백 2~6px. 메타·본문 사이 8~12px. 본문·액션 사이 16~20px.
- 카드 라운드: `rounded-xl` (12px) 일관. 큰 카드(메인 CTA)만 `rounded-2xl` (16px).

---

## 6. 리스트 아이템

```
[● 색도트] [메타: 과목] [배지: D-2]
          [본문 한 줄: 자료구조 과제 — 이진 탐색 트리]
                                              [hover: 액션 →]
```

- 도트: 과목 색 (sidebar 색과 일치)
- 배지: 위 §1-2 임팩트 색 규칙대로
- 호버 시에만 우측 액션 노출 (Linear·Granola 스타일)
- 행 높이: 56~64px (`py-4`)

---

## 7. 버튼

| 변형 | 용도 | 스타일 |
|---|---|---|
| Primary (어두운) | 카드 메인 CTA ("시작") | `bg-fg text-white` |
| Accent | 결제·전역 CTA | `bg-accent text-white` |
| Ghost | 행 호버 액션 | `text-fg-muted` (호버 시 노출) |
| Outline | 보조 액션 | `border bg-white` |

**규칙**:
- Primary는 한 화면에 최대 2개.
- 같은 위계의 버튼이 3개 이상 보이면 위계를 다시 짜라.
- 모서리: 8px (`rounded-lg`) 일관.

---

## 8. 여백 리듬 (★ 단조로움 깨기)

같은 섹션 타입을 절대 두 번 연달아 X. 우리 Today 페이지는 이미 잘 되어있음:

```
[히어로 카드: 큰 그라데이션] ← 강조
   ↓
[리스트 카드 3개]            ← 평면
   ↓
[리스트 카드 1개]            ← 평면
   ↓
[리스트 카드 3개]            ← 평면
```

3번째·4번째가 같은 형태라 단조롭다 — **5번째 섹션에는 다른 패턴**(예: 가로 스크롤 카드, 큰 일러스트, 통계 위젯)을 넣어 리듬을 깬다.

페이지당 섹션 4~7개. 4 미만이면 빈 페이지, 7 초과면 산만함.

---

## 9. 마이크로카피 톤 (한국어)

**원칙**: 친근한 반말 X, 정중한 평어 O. ChatGPT·Toss 스타일.

| 좋음 | 나쁨 |
|---|---|
| "오늘 5분이면 끝나는 일부터" | "5분 안에 빠르게 끝내세요!" |
| "지금 시작" | "지금 바로 시작하기" |
| "확인이 필요해요" | "주의: 확인 필요!" |
| "복습 12문제" | "12개의 복습 문제가 있습니다" |

- 능동태, 간결, 긍정형
- 느낌표 페이지당 0~1개
- "여러분"·"우리" 자제. "OO님"으로 직접 호명

---

## 10. 절대 금지 (StyleSeed의 30+ Don'ts 중 핵심)

- ❌ `#000` 검정 (가장 진한 회색은 `#2A2A2A`)
- ❌ 진한 그림자 (`0 4px 8px rgba(0,0,0,0.1)` 같은)
- ❌ 액센트 색 두 개 이상
- ❌ 본문에 액센트 색
- ❌ 큰 면적에 경고/임팩트 색
- ❌ 한 카드에 4개 초과 정보
- ❌ 페이지에 같은 카드 형태 4개 연달아
- ❌ 무지개 색 (5색 이상 차트)
- ❌ 라운드 16px 초과 (이미지·아바타 제외)
- ❌ AI 생성 일러스트·이모지 본문 사용 (섹션 헤더 작은 아이콘만 OK)
- ❌ 그라데이션 남용 (히어로 카드 1곳만)
- ❌ 폰트 5종류 이상
- ❌ **이벤트·태그·뱃지 좌측 동그라미 점(`●`)** — Google Calendar식 클리셰. AI 티 팍팍 남.
  - 대신: 색 자체로 카테고리 표현 (배경·텍스트 색) 또는 좌측 얇은 vertical bar(2~3px)
- ❌ **마침표 끝 카피 패턴 남용** ("9 강의.", "이번 학기 일정.", "막혔을 때 꺼내는 12개 도구.")
  - 강한 정체성 카피 1~2곳만. 모든 헤더·카드에 박으면 AI 톤
- ❌ **shadcn/Linear 클론 톤 모달** — 백드롭 blur + 흰 카드 + 회색 헤더 + X 버튼.
  - arch 모달은 우리 색·여백·타이포·corner 라운드를 따라야. Generic 헤더+바디 분리 X
- ❌ **풀 라운드 pill segmented control 무분별 사용** — 액센트 위치에만

---

## 10-B. 살아있는 톤 가드 (2026-05-23 추가) ★

캘린더 리뉴얼에서 검증된 "살아있는" 디자인 토큰들. 다른 화면 만들 때 이 톤을 빌려와
정체성 일관 유지. shadcn 박스 톤이 의심되면 아래 세 패턴을 끼워넣어 즉시 살림.

### 10-B-1. Sparkles orb (AI 진입의 1급 정체성)

AI가 개입하는 진입점은 모두 같은 orb로 표시. 화면 간 "AI"가 같은 얼굴로 보임.

```tsx
<span
  aria-hidden
  className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-white shadow-[0_2px_8px_-2px_rgba(0,113,227,0.4)]"
  style={{
    background:
      "linear-gradient(135deg, var(--color-apple-action) 0%, #7aa6d6 60%, #a08bc4 100%)",
  }}
>
  <Sparkles className="h-[15px] w-[15px]" strokeWidth={2.2} />
</span>
```

적용 위치: `start-screen` 챗 입구, `tools` AI 카드, `calendar` AI 진입 카드, `chat` AssistantAvatar.
❌ 검정 동그라미 + 글자 한 자 (`a`, `i` 등) — 이미 제거됨.

### 10-B-2. Typing placeholder (입력 어포던스 살아있게)

빈 입력칸은 죽은 회색 placeholder 대신 prompt 4~5개가 글자 단위로 타이핑·삭제되며 회전.
사용자가 입력 시작하면 즉시 사라짐. "이런 식으로 적으면 되는구나" 학습 + 화면이 정지하지 않은 느낌.

핵심 동작: `phase: "typing" | "hold" | "deleting"` 상태머신 + setTimeout 50~55ms 글자 추가, 24ms 글자 삭제, 1400ms hold.

적용: `start-screen`, `tools/tools-entry-card`, `calendar/ai-entry-card`.

### 10-B-3. 좌측 ribbon + hover glow (.card-glow-ribbon)

카드형 컴포넌트의 카테고리·임박도 식별. 캘린더 EventChip의 좌측 얇은 색 bar를 카드 크기로 확장.

```tsx
<article
  className="group card-glow-ribbon relative overflow-hidden rounded-[14px] bg-white p-5"
  style={{ ["--ribbon-color" as string]: "#e0445e" }}
>
  ...
</article>
```

자동으로:
- 좌측 2.5px ribbon → hover에서 3.5px
- 좌측에 옅은 glow가 hover에서 번짐
- 카드가 -2px 살짝 떠오름

적용 위치: today `UpcomingCard`, study `CourseCard`·`materials-grid`, review `ReviewCard`, chat suggestion 카드.

### 10-B-4. .urgent-pulse · .clock-tick (시간 살아있게)

임박한 D-day·카운트다운은 정적 텍스트가 아니라 미세 박동 + 숫자 교체 애니메이션으로 표시.

- `.urgent-pulse`: 1.8s 부드러운 opacity 박동 (D-day urgent, 카운트다운 6시간 이내)
- `.clock-tick`: 숫자 값 변할 때 위에서 살짝 떨어지며 blur가 빠지는 디지털 톤

적용: today `TodayHero` ClockCell, `UpcomingCard` D-day.

### 10-B-5. fade-up stagger

페이지 진입 시 섹션이 위로 살짝 올라오며 등장. 첫 인상을 살아있게.

```tsx
<header className="fade-up fade-up-1">...</header>
<section className="fade-up fade-up-2">...</section>
<section className="fade-up fade-up-3">...</section>
```

globals.css에 `fade-up-1`~`fade-up-5` 지원. 위에서 아래로 30~60ms 간격으로 stagger.

---

## 11. 새 페이지 만들 때 체크리스트

- [ ] 액센트 색이 단 하나인가?
- [ ] 검정(#000)을 안 썼는가?
- [ ] 한 카드 정보 4개 이하인가?
- [ ] 같은 카드 형태가 4개 이상 연달아 나오지 않는가?
- [ ] 그림자가 보일 듯 말 듯한가?
- [ ] 텍스트 크기 5단계 이내인가?
- [ ] tabular-nums가 숫자에 적용됐나?
- [ ] 한국어 마이크로카피가 정중한 평어인가?
- [ ] 비어 있을 때 다음 액션을 제안하나? (Empty state)
- [ ] **모바일(375px)·태블릿(820px)·데스크톱(1280px) 셋 다 확인했나?**
- [ ] **터치 타깃 44px 이상인가?**
- [ ] **`md:` 경계(768px)에서 상단바 ↔ 하단 탭바가 스왑되는가?**
- [ ] **§10-B 살아있는 톤 적용?** — AI 진입은 Sparkles orb, 카테고리 카드는 `.card-glow-ribbon`, 시간 임박은 `.urgent-pulse`, 진입은 `fade-up` stagger

---

## 12. 참고 자료

- [STYLESEED-FULL.md](STYLESEED-FULL.md) — bitjaru/styleseed 원본 룰북 (2670줄). 위 가이드로 부족할 때 검색
- [styleseed-linear-tokens.css](styleseed-linear-tokens.css) — Linear 스킨 토큰 참고용 (적용은 우리 globals.css가 담당)
- [styleseed-toss-tokens.css](styleseed-toss-tokens.css) — Toss 스킨 (한국 SaaS 톤 참고)
- 라이브 데모: https://styleseed-demo.vercel.app (Linear/Toss/Raycast 스킨 토글)

---

## 13. 반응형 전략 (★ 1급 시민)

대학생은 **모바일·태블릿·데스크톱 다 씀**. 단일 코드베이스, 화면 크기로만 분기.

> ⚠️ 이 섹션은 코드 실측(2026-07-16) 기준으로 다시 썼다. 이전 버전은 폐기된
> 사이드바 설계와 실제로 존재한 적 없는 컨테이너·폰트 수치를 담고 있었다.
> 표의 숫자를 바꿀 땐 반드시 코드를 grep해서 맞춘다 — 문서가 거짓이면 감사가 무의미하다.

### 13-1. 브레이크포인트 — 문서 정책 vs 코드 현실

| 토큰 | 너비 | 문서상 의도 | **실제 코드** |
|---|---|---|---|
| (none) | < 640px | 모바일 풀 | 모바일 풀 |
| `sm:` | ≥ 640px | 여백만 살짝 | **레이아웃 대부분이 여기서 넓어짐** (516회 — 사실상 데스크톱 진입) |
| `md:` | ≥ 768px | 태블릿 | **네비 스왑 경계** (상단바 ↔ 하단탭바, 70회) |
| `lg:` | ≥ 1024px | 데스크톱 진입 | 거의 미사용 (55회, 대부분 사소한 노출 토글) |
| `xl:` | ≥ 1280px | 데스크톱 풀 | **패딩만 (10회, `xl:px-*`뿐)** — "데스크톱 풀"은 이미 sm/md에서 끝남 |

**현실 요약**: 의도한 5단계가 사실상 2단계로 붕괴돼 있다 — `sm:`(640)에서 넓어지고 `md:`(768)에서 네비가 바뀐다. `lg:`/`xl:`은 장식용 패딩뿐. 새 페이지를 이 표의 "의도"대로 5단계로 짜면 기존 페이지와 어긋난다. **일치시키려면 별도 리팩터 필요** (알려진 부채).

### 13-2. 네비게이션 — 상단바 ↔ 하단 탭바 (사이드바 없음)

2026-05-31에 좌측 사이드바를 제거하고 상단바로 대체했다 (`sidebar.tsx`는 import 호환용 noop stub만 남음). "이번 학기 과목" 강의 트리도 이때 사라졌다.

- **`md:` 이상 (≥768)**: `GlobalTopbar` — 중앙 6항목 세그먼트 pill(홈·공부·내 문제·복습·일정·도구) + 우측 학기칩·테마·프로필. `hidden ... md:block` ([global-topbar.tsx:42](../../src/components/global-topbar.tsx#L42))
- **`md:` 미만 (모바일)**: `MobileTopbar`(h-11 미니 헤더) + `MobileTabBar`(h-14=56px, **4탭**: 홈·공부·일정·도구). 둘 다 `md:hidden` ([mobile-nav.tsx:128,154](../../src/components/mobile-nav.tsx#L128))
- 하단 탭바는 iOS Safe Area 반영 (`pb-[env(safe-area-inset-bottom)]`)
- 데스크톱 네비 6항목 vs 모바일 탭 4항목 — 모바일에선 "내 문제·복습"이 탭에서 빠짐 (홈/공부 하위로 접근)

### 13-3. 컨테이너 너비 (실측)

대시보드는 한 컬럼 유지 (정보 밀도상 데스크톱에서도 한 컬럼이 맞음 — ChatGPT·Claude 패턴). 단, **셸이 통일돼 있지 않다** — 공용 셸과 페이지별 hand-rolled가 섞여 있다.

**공용 `AppleShell`** ([apple-shell.tsx](../../src/components/apple-shell.tsx)) — study·quiz·history·review 4개 페이지에서만 사용:
- `narrow` 820 / `default` 1080 / `wide` 1200 / `hero` 1440
- 패딩: `px-6 sm:px-10 md:px-12`

**`PageShell`은 정의만 있고 `<PageShell>` 렌더 사용처 0** — dead. (`PageTitle` 등 헬퍼만 일부 사용)

**hand-rolled (셸 우회) — 주요 페이지 다수가 여기 해당**:
- 대시보드 홈: `max-w-[1440px]` ([page.tsx:45](../../src/app/dashboard/page.tsx#L45))
- 강의자료 상세: `max-w-[920px] md:max-w-[1400px]` — **md:에서 920→1400 점프** ([[material]/page.tsx:103](../../src/app/dashboard/study/[course]/[material]/page.tsx#L103))
- 캘린더: `max-w-[1280px]` ([calendar/page.tsx:47](../../src/app/dashboard/calendar/page.tsx#L47))
- 도구 허브: `max-w-[1080px]` ([tools/page.tsx:213](../../src/app/dashboard/tools/page.tsx#L213))

실제 max-width는 **920~1440px** 범위, 패딩은 `px-5/6 → sm:px-8/10 → md:px-10/12`. (이전 문서의 672/768px·`px-4`는 존재한 적 없음.)

### 13-4. 카드·리스트 모바일 변환

- 데스크톱의 호버 노출 액션 → 모바일에선 **항상 노출** 또는 행 자체가 탭(전체 행 터치 영역)
- D-day 배지는 그대로 (작아도 가독). **카테고리 표시는 도트(`●`) 금지 — §10대로 색 자체 또는 2~3px vertical bar** (2026-07-16 import flow에서 도트 제거 완료)
- 카드 내부 padding: 데스크톱 `p-5~p-6` → 모바일 `p-4`
- 5분 메인 카드의 가로 정렬(`flex items-end justify-between`)은 모바일에서 `flex-col gap-4`로 분기

### 13-5. 폰트 크기 (실측)

대학생은 작은 폰을 가까이 본다 — 모바일에서 과하게 키우지 않되, **코드는 고정 `text-[Npx]` + `sm:`/`md:` 단계별 증가**를 쓴다 (모바일/데스크톱 2단이 아님).

| 레벨 | 실제 패턴 (예시) | 비고 |
|---|---|---|
| Display(히어로) | `text-[19~28px] sm:text-[32~36px] md:text-[38~42px]` | 최대 **42px**까지 감 |
| H1(페이지) | `text-[24~28px] sm:text-[30~36px] md:text-[34~42px]` | |
| 섹션 헤더 | `text-[17px] sm:text-[19px]` | |
| Body | `text-[13.5px] sm:text-[14.5px]` | **sm:에서 올라감** (고정 아님) |
| Caption | `text-[11~12px]` | |

수치가 페이지마다 조금씩 다르다 (통일 안 됨). 새 화면은 위 범위 안에서 인접 페이지를 grep해 맞춘다. (이전 문서의 "Body 13.5px 동일"은 틀림 — sm:에서 14~14.5px로 오름.)

### 13-6. 터치 타깃

- **모든 인터랙티브 요소 44×44px 이상** (Apple HIG 기준)
- 체크박스·도트·아이콘 자체는 작아도, 터치 영역은 패딩으로 확보
- 인접 버튼 사이 8px 이상 간격
- ⚠️ 미검증: 일부 `h-7`(28px)·`h-[30px]` 버튼이 산재 (split-control, page chip 등). 다만 상당수가 `md:`-only(마우스 맥락)라 실제 위반 여부는 화면별 판단 필요.

### 13-7. iPad 현황 (특이 처리 없음)

- **iPad 전용 분기 없음.** iPad 세로(768~1024)는 `md:` 티어에 그대로 들어가 데스크톱 상단바 + 데스크톱 컨테이너 너비(강의자료는 md:에서 1400px)를 받는다.
- `md`↔`lg` 사이에 바뀌는 건 학기칩 노출(`lg:inline-flex`) 정도뿐. iPad 가로용 `lg:` 레이아웃 정제 없음.
- 사이드바가 없으므로 이전 문서의 "옵션 A(압축 사이드바)"는 무의미 — 폐기.
- **개선 여지 (부채)**: iPad 세로 820px에서 1400px 컨테이너는 과할 수 있음. 필요 시 `md:`와 `lg:` 사이 중간 너비 도입 검토.
- iPad 외부 키보드·트랙패드 — 호버 상태 정상 동작 확인 필요.

### 13-8. 검증 워크플로 — 새 페이지 끝낼 때마다

```bash
# 1. 데스크톱 (이미 보고 있음)
# 2. 태블릿: Chrome DevTools → iPad Air (820×1180) — md: 티어 확인
# 3. 모바일: Chrome DevTools → iPhone 14 (390×844) — 하단 탭바 확인
```

Playwright MCP 설치됨 — 한 줄로:
- "iPhone 14, iPad Air, Desktop 1280에서 **/dashboard** 스크린샷 찍어줘" (라우트는 `/dashboard`, `/dashboard/today` 아님)
- 라이트/다크 양쪽 + 호버·탭·키보드 인터랙션 자동 검증
- 토큰 실측: `getComputedStyle(document.documentElement).getPropertyValue('--color-*')`로 렌더값 직접 확인 (2026-07-16 토큰 드리프트 잡을 때 쓴 방법)

---

**문서버전**: 2026-07-16 (§13 코드 실측 기준 전면 재작성)
**알려진 부채**: (1) `sm:` 과부하로 5단계 붕괴 (2) 셸 미통일 — AppleShell vs hand-rolled (3) `bg-white` 312곳 시맨틱 토큰화 (4) 폰트 수치 페이지별 편차
